...
export class HomeComponent implements OnInit {
    ...
    // default constructor
    constructor(public globalsService: GlobalsService, private appConfigService: AppConfigService, private apiDataAccessService: ApiDataAccessService, private sessionsDataService: SessionsGridDataService, private logsDataService: LogsGridDataService, http: HttpClient, apiGatewayService: ApiGatewayService) {
        // set current page
        this.globalsService.CurrentPage = "home";

        // global filter updated event
        this.globalFilterUpdateSubscription = this.globalsService.globalFilterUpdated$.subscribe(
            astronaut => {
                alert('global filter updated - from home screen');
            });

        

        this.viewLogs = logsDataService;
        //this.logsDataService.entityName = "Logs";
        this.logsDataService.query(this.stateLogs, { fromDate: "2016-01-01", toDate: "2021-01-01", serverCode: 0, userCode: 0, processCode: 0, pluginCode: 0, logLevel: 0 });

        this.viewSessions = sessionsDataService;
        //this.sessionsDataService.entityName = "Sessions";
        this.sessionsDataService.query(this.stateSessions, { datesRangeType: "This Month" });
    }

    public dataStateChangeLogs(state: DataStateChangeEvent): void {

        console.log('starting state change (logs)...');
    ...
    public dataStateChangeSessions(state: DataStateChangeEvent): void {
        ...
        console.log('starting state change (sessions)...');
    ...
    reloadApplicationsStatistics() {
        this.apiDataAccessService.applications_getApplicationsStatistics('').then((data: any) => {
...
export abstract class NorthwindService extends BehaviorSubject<GridDataResult> {
    ...
    protected fetch(tableName: string, state: any): Observable<GridDataResult> {
        ...
        return this.http
            .get(`/applications/applications-statistics`)
            .pipe(
                map(response => (<GridDataResult>{
                    data: response['ResponseData'],
                    total: parseInt(response['ResponseData'].length, 10)
                })),
                tap(() => {
                    this.loading = false;
